...
|---|---|---|---|
| `share2brain:discord:messages` | bot | `share2brain:indexer` *(consumer activo desde 3.3)* | workers/indexer |
...
| `share2brain:knowledge:events` | bot (desde 3.2: `discord.backfill.completed`); workers *(Epic 6)* | `share2brain:notifier` | notifier *(deferred — Epic 6)* |
...
sin fila en `embeddings`). El bloque `enrichment` requerido en `Share2BrainConfigSchema` (`config/
...
| Logging | Nivel configurado via `observability.log_level` en `Share2Brain.config.yml`; todos los servicios usan el logger exportado desde `packages/shared` (Story ops-4: `bot`/`workers` migrados a él, copias locales eliminadas). Dual sink: cada línea ≥ umbral va a `stdout` **y** a un `StructuredLogSink` vendor-neutral inyectado — el sink del puerto `Observability` (Story ops-5). El logger **no importa `@sentry/node`**; el adaptador Sentry reenvía a Sentry Structured Logs, el Noop (DSN vacío, S-5) descarta |
...
- **Estrategia de batching del Indexer *(retirado, Epic 7 — Stories 7.2/7.3):*** El diseño de Story 3.3 agrupaba entradas de un batch `XREADGROUP` por `channelId` en grupos de ≤ `grouping_window` mensajes y las troceaba con `RecursiveCharacterTextSplitter` (`chunk_size`/`chunk_overlap`). El pivote a índice curado de recursos retira esa agrupación/chunking tanto del Indexer (Story 7.2) como del Sync Worker (Story 7.3): cada mensaje se procesa individualmente, una fila de `embeddings` por URL extraída, y una edición reconcilia por diff de links en vez de re-chunkear. `chunking.ts` y el bloque `knowledge.*` de la config compartida ya no existen en el repositorio.
- **Observabilidad detallada:** **Resuelto en Story ops-4; refactorizado a puerto + adaptador en Story ops-5.** La observabilidad se expone como un **puerto vendor-neutral** `Observability` (interfaz en `packages/shared/src/observability/observability.ts`, espejo del puerto `Notifier`), con Sentry como **un adaptador** detrás de la fábrica `createObservability({ dsn, service, provider })` y un `NoopObservability` para el caso DSN vacío (S-5). `@sentry/node` se importa en **un único archivo** — `packages/shared/src/observability/sentry.ts` (AD-2, endurecido): los servicios dependen del **puerto**, nunca de `@sentry/node`, y el logger tampoco lo importa (recibe un `StructuredLogSink` inyectado). Cada `main.ts` llama a `createObservability(...)` **justo después de `loadConfig()` y antes de cualquier I/O de red** (AD-8) y pasa su `logSink` al `createLogger`. Alcance: **logs completos + errores**, no performance tracing. El adaptador captura `uncaughtException`/`unhandledRejection` y los 5xx del backend vía `setupExpressErrorHandler`. Nunca se envía PII/`content` ni se activa `sendDefaultPii`: `beforeSend`/`beforeSendLog` pasan mensaje, stack y atributos por `redactSecrets` y descartan cualquier `content`; el contexto de usuario es solo id interno + roles. **Añadir un proveedor** (OpenTelemetry, Datadog, …) = nuevo archivo adaptador + una rama en `createObservability` + un valor de config `observability.provider`, **sin tocar** `backend`/`bot`/`workers`/`web`. Performance tracing con un `instrument.ts` de import-time queda deferred hasta que se necesiten spans de request.
  - **Trazado de inferencia LLM (Story ops-6):** es un **puerto SEPARADO** `LlmTracing` (`packages/shared/src/tracing/`), **no** el puerto `Observability` (D1, Interface Segregation): Sentry mantiene errores+logs; Arize Phoenix (self-hosted) recibe las trazas LLM. Mismo patrón que ops-5 (puerto + Noop + fábrica `createLlmTracing({ endpoint, service, provider })` + `NoopLlmTracing` con `endpoint` vacío ⇒ Noop, S-5, contrato never-throw). Los SDKs `@opentelemetry/*`/`@arizeai/*` viven en **un único archivo** — `packages/shared/src/tracing/phoenix.ts` (AD-2, regla grep-green como `@sentry/node`); el módulo **no** se re-exporta desde el barrel raíz (arrastra OTel+LangChain, igual que `providers`). Solo `backend`+`workers` (el bot no hace llamadas LLM, D3). El **contenido** (prompt/completion/fragmentos) puede viajar en spans **solo** porque el colector es self-hosted dentro de la red del Compose y nunca expuesto públicamente (SNF-18); la prohibición hacia Sentry (SNF-9) permanece intacta. Auto-instrumentación OpenInference de `@langchain/core` (reason/compress/enrich) + `withSpan` manual para embeddings y la query pgvector.
