...
| Logging | Nivel configurado via `observability.log_level` en `Share2Brain.config.yml`; todos los servicios usan el logger exportado desde `packages/shared` (Story ops-4: `bot`/`workers` migrados a él, copias locales eliminadas). Dual sink: cada línea ≥ umbral va a `stdout` **y** a Sentry Structured Logs cuando `observability.sentry_dsn` está presente (no-op si vacío, S-5) |
...
- **Observabilidad detallada:** **Resuelto en Story ops-4.** `@sentry/node` vive **solo** en `packages/shared` (AD-2); los tres servicios lo heredan transitivamente y cada `main.ts` llama a `initSentry(config.observability.sentry_dsn, '<service>')` **justo después de `loadConfig()` y antes de cualquier I/O de red** (AD-8) — no-op cuando el DSN está vacío (S-5). Alcance elegido: **logs completos + errores**, no performance tracing. El logger compartido reenvía **cada línea** (dual sink: stdout + `Sentry.logger`, respetando `observability.log_level`) además de capturar los `uncaughtException`/`unhandledRejection` y los 5xx del backend vía `setupExpressErrorHandler`. Nunca se envía PII/`content` ni se activa `sendDefaultPii`: `beforeSend`/`beforeSendLog` pasan mensaje, stack y atributos por `redactSecrets` y descartan cualquier `content`; el contexto de usuario es solo id interno + roles. Performance tracing con un `instrument.ts` de import-time queda deferred hasta que se necesiten spans de request.
