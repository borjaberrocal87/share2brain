---
description: Backend development standards, best practices, and conventions for the Share2Brain Node.js/TypeScript monorepo — hexagonal shared-kernel architecture, DDD principles, SOLID+DRY design, Drizzle/pgvector, Express 5 API, LangGraph RAG agent, Redis Streams ingestion, testing standards (Vitest), and security.
...
This document outlines the conventions and standards for the Share2Brain backend-side services. "Backend" here spans three runtime processes plus two library packages:

- `@share2brain/shared` — domain kernel (Drizzle schema, Zod API schemas, `loadConfig()`, shared types).
- `@share2brain/web` — Vite + React SPA (built static, served by nginx; see `docs/frontend-standards.md`).
- `@share2brain/bot` — Discord Bot, ingestion producer.
- `@share2brain/backend` — Express API + LangGraph RAG agent.
- `@share2brain/workers` — Indexer + Sync consumers.
...
import { something } from '@share2brain/backend'   // from bot/workers/web
import { something } from '@share2brain/bot'        // from backend/workers/web
```
...
```
packages/
  shared/                 # @share2brain/shared — domain kernel
    src/
      db/
      ...
      config/
        index.ts          # loadConfig() + Zod schema for Share2Brain.config.yml
      types/
...
- **AD-2 — shared kernel, no cross-service imports**: shared types/schemas/config live in `@share2brain/shared`.
...
The following patterns define the target domain model. Not all code currently follows these patterns — migrate incrementally. The shared kernel (`@share2brain/shared`) is the natural home for shared entities and value objects.
...
export class RbacService {
  constructor(
    private db: Database,
    private config: Share2BrainConfig,
  ) {}
...
Domain events handle side effects of domain operations in a decoupled manner. Share2Brain uses Redis Streams as the domain event bus — e.g., a `MessageCreatedEvent` published to `share2brain:discord:messages` is a domain event.
...
function startService() {
  const config = loadConfig('Share2Brain.config.yml')
...
export function loadConfig(path?: string): Share2BrainConfig { /* ... */ }
...
- **Packages**: `@share2brain/{bot,backend,workers,web,shared}`.
- **REST endpoints**: `/api/<resource>` kebab-case plural; route params in camelCase (`/api/conversations/:conversationId`).
- **Redis stream keys / consumer groups**: fixed by AD-13 (`share2brain:discord:messages`, group `share2brain:indexer`, etc.).
...
- Use the unified error shape from `@share2brain/shared/schemas/errors.ts`: `{ error: string, code: string }` (AD Consistency Conventions).
...
- Use the logger exported from `@share2brain/shared`; level from `observability.log_level` in `Share2Brain.config.yml`.
...
|---|---|---|---|---|
| `share2brain:discord:messages` | bot | `share2brain:indexer` *(consumer live since 3.3)* | workers/indexer |
...
| `share2brain:knowledge:events` | bot (since 3.2: `discord.backfill.completed`); workers *(Epic 6)* | `share2brain:notifier` | notifier *(deferred — Epic 6)* |
...
- `channel_permissions` is materialized from `Share2Brain.config.yml` via upsert at Backend startup, before accepting requests.
...
- **Module-level**: `vi.mock('@share2brain/shared/db')` at the top of test files.
...
### Share2Brain-Specific Testing Requirements
...
- **Secrets only in `.env`; behavior only in `Share2Brain.config.yml`** — never mix (AD Consistency Conventions). Never commit `.env`.
...
```bash
npm run dev -w @share2brain/backend    # API + agent, hot reload
npm run dev -w @share2brain/bot        # Bot
npm run dev -w @share2brain/workers    # Workers
npm run lint                     # ESLint
npm run test                     # Vitest
...
This document is the foundation for a maintainable, scalable, and testable Share2Brain backend. All contributors follow these practices; when in doubt, the `AD-*` invariants in `docs/context/ARCHITECTURE-SPINE.md` win.
