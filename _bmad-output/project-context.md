...
- **Observability = Sentry, single integration point in `shared`** (Story ops-4, AD-2): `@sentry/node` is a dependency of `@share2brain/shared` **only**; services import from `@share2brain/shared/observability`, never `@sentry/node` directly. Call `initSentry(config.observability.sentry_dsn, '<service>')` in `main.ts` **right after `loadConfig()` and before any network I/O** (AD-8) — a no-op when the DSN is empty (S-5). The shared logger is a **dual sink** (stdout + Sentry Structured Logs, gated by `observability.log_level`). **Never send PII or message `content`**: `beforeSend`/`beforeSendLog` scrub message/stack/attributes via `redactSecrets` and drop any `content`; user context is internal id + Discord roles only; `sendDefaultPii` stays off.
