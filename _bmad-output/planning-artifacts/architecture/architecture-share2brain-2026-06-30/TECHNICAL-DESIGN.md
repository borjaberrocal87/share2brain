...
**Indexer** — consume `share2brain:discord:messages` (consumer group `share2brain:indexer`):
...
3. XACK share2brain:discord:messages share2brain:indexer <message-id>
```

**Sync Worker** — consume `share2brain:discord:messages:updated` y `share2brain:discord:messages:deleted` (consumer group `share2brain:sync`):
...
|---|---|---|---|---|
| `share2brain:discord:messages` | bot | `share2brain:indexer` | workers/indexer | Indexar mensajes nuevos |
...
| `share2brain:knowledge:events` | workers/bot | `share2brain:notifier` | notifier *(deferred)* | Notificaciones al operador |
