...
UX-DR11: **Cards de resultado de búsqueda** — Card con padding 18px 20px, background --surface, border --border, border-radius 14px, hover border-strong. Fila superior: [badge canal amber (mono 12px, rgba(245,166,35,0.1) bg) | fecha (12px --tx5)] / [similarity bar 54x5px (amber gradient, border-radius 3px, track bg) | porcentaje (mono 11.5px)]. Contenido: 14.5px, line-height 1.6, --tx. Fila inferior: [avatar autor 24px + nombre] / [enlace "ver en Discord" con ícono externo, hover #5865F2].

UX-DR12: **Vista Documentos — tabla y controles** — Header: título + descripción ("El punto ámbar marca las fuentes sin leer — tocá una fila para marcarla como leída"). Barra de controles: filter chips + spacer + botón "Marcar todas como leídas" (pill, visible solo si hay no-leídos) + chip toggle "Sin leer · N" (mismo style que chips de canal pero con dot amber). Tabla: border-radius 14px, overflow hidden. Header tabla: grid 4 cols, uppercase mono 10.5px --tx5, bg --bg. Filas: hover --hover-row. Paginación: "mostrando X de Y · N sin leer" + botón "Cargar más" (border amber en hover).

UX-DR13: **Filas de tabla de documentos** — Grid `1fr 130px 130px 96px`. Columna chunk: dot (7px) + texto clamped 2 líneas. Dot ámbar con glow `0 0 0 3px rgba(245,166,35,0.16)` = no leído. Dot --dot-read sin glow = leído. Texto no leído: --tx, weight 500. Texto leído: --tx4, weight 400. Columna canal: mono 12px amber. Columna autor: avatar 20px + nombre truncado 12.5px. Columna fecha: mono 11.5px --tx4, text-align right. Click en fila marca como leído.
...
UX-DR21: **Chat — citas/fuentes** — Sección "Fuentes" (mono uppercase 10px --tx5). Chips en flex-wrap gap:8px. Cada chip: avatar 20px + badge canal mono amber + nombre autor + ícono externo. Hover border #5865F2. Links abren en `_blank`.
