...
Ran `npx vitest run --project backend-integration` repeatedly, with live `docker compose` app containers (`hivly-backend/bot/workers`) up (they had been running ~4h against the same Postgres/Redis), then with those three containers stopped.
...
- **Fail-fast guard (AC-4):** `assertNoCompetingWriter(db)` in `openTestClients` throws with an actionable message if a foreign connection to the test DB is detected (different `client_addr` in `pg_stat_activity`) — catches a live `docker compose` app container attached to the same DB. Best-effort + false-positive-free on a clean/CI DB (empty → passes); bypass with `HIVLY_TEST_ALLOW_SHARED_DB=1`. Note: an idle app (closed pool) shows no connection, so the DOC is the primary mitigation.
