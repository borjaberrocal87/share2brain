...
This is the **fourth and final story of Epic 6** (Synchronization, Notifications & Reliability) and the last feature story before the epic retrospective. It is **cross-cutting** — it touches all three long-running services (`bot`, `backend`, `workers`), the shared kernel (`@hivly/shared`), the config schema, `Hivly.config.yml`, `.env.example`, and `docker-compose.yml`. It has three independent concerns, each mapped to one epic AC:
...
5. **`config.security.rate_limit` is reshaped to three tiers (CONFIRMED with Borja, DECISION 2) — this is a `@hivly/shared` schema change (this story legitimately owns it).** Old shape `{ window_ms, max_requests }` → new shape `{ api: {window_ms,max_requests}, auth: {…}, chat: {…} }`, seeded in `Hivly.config.yml` with the epic's exact numbers (API 100/15min, auth 10/15min, chat 20/min). The Zod schema (`packages/shared/src/config/index.ts`) and the existing `config/index.test.ts` fixture must both be updated. `security.allowed_origins` is untouched. **Trust-proxy is required for per-IP counting (note #6).**
...
13. **Logger: add canonical `@hivly/shared/logger`, use in backend only (CONFIRMED with Borja, DECISION 3).** The bot & workers loggers are byte-identical dupes (only the `[bot]`/`[workers]` prefix differs). Promote the implementation to `packages/shared/src/logger.ts` exported as `@hivly/shared/logger` (subpath export, mirror `@hivly/shared/redis`), parameterizing the service prefix. The **backend** uses it for the new shutdown/notifier paths (`createLogger(config.observability.log_level, 'backend')`). **Leave bot & workers on their local loggers** (mature 6.1/6.2 code — don't churn) and **leave the other ~34 backend `console.*` sites untouched** (out of scope). The remaining duplication is accepted, documented debt.
...
**Given** `loadConfig()`
**When** it validates `Hivly.config.yml`
...
**And** `Hivly.config.yml` and `.env.example` are updated with the new keys (behavior in YAML referencing `${TELEGRAM_BOT_TOKEN}`/`${TELEGRAM_CHAT_ID}`/`${SLACK_WEBHOOK_URL}`; the raw secrets documented in `.env.example`).
...
- [x] **Task 4 — Backend: helmet + tiered rate limiting** (AC-2, note #4, #6, #10, #11) — `packages/backend/src/app.ts` (+ `main.ts`):
  - [x] `npm install helmet@^8 express-rate-limit@^8 -w @hivly/backend` (peer `express >= 4.11` — Express 5 OK). Add to `packages/backend/package.json` dependencies.
...
- [x] **Task 6 — Bot & workers: additive notifier wiring only** (AC-1, note #8, #9) — `packages/bot/src/main.ts`, `packages/workers/src/main.ts`:
  - [x] In each `main()`, after `createLogger(...)`: `const notifier = createNotifier(config.notifications, logger)` (import `createNotifier` from `@hivly/shared/notifier` — allowed; it's shared, AD-2).
...
- [x] **Task 8 — Config file + env docs** (AC-5, note #2) — `Hivly.config.yml`, `.env.example`:
  - [x] `Hivly.config.yml`: add the top-level `notifications` block (enabled + provider + `${...}`-referenced creds for the active provider) and reshape `security.rate_limit` into the three tiers with the epic's numbers (API 100/900000ms, auth 10/900000ms, chat 20/60000ms). Comment the config-vs-secrets rule.
...
- **Shared adapters (Notifier, logger) mirror `@hivly/shared/redis` (`packages/shared/src/infrastructure/redis.ts`).** A factory that constructs but performs NO I/O at construction, attaches internal error handling so a failure degrades rather than crashes, and exports an inferred/explicit type. Subpath-only export (NOT in the root barrel `src/index.ts`) — the barrel intentionally omits side-effect/heavy adapters (see `providers/index.ts:8-11`). AD-2: all three services may import `@hivly/shared/notifier` and `@hivly/shared/logger`; none imports another service.
...
- **Notifications: 100% new.** No `telegram`/`slack`/`webhook`/`notify` code exists; Sentry is an unwired config key only. The `hivly:knowledge:events` stream + `hivly:notifier` group are *reserved* in `types/events.ts` but deliberately **not** used this story (note #1).
...
- **NEW** `packages/shared/src/logger.ts` + `logger.test.ts`; export `@hivly/shared/logger`
- **NEW** `packages/shared/src/notifier/index.ts` + `notifier.test.ts`; export `@hivly/shared/notifier`
...
- **UPDATE** `Hivly.config.yml`, `.env.example`
...
- [Source: _bmad-output/planning-artifacts/architecture/architecture-hivly-2026-06-30/ARCHITECTURE-SPINE.md — AD-1 (per-process lifecycle), AD-2 (shared kernel / no cross-service import), AD-7 (nginx sole exposed entrypoint), AD-8 (central loadConfig), AD-13 (streams + reserved hivly:notifier group); Consistency Conventions (secrets vs config); Deferred § (notifier placement open)]
- [Source: _bmad-output/planning-artifacts/architecture/architecture-hivly-2026-06-30/TECHNICAL-DESIGN.md#§8 (streams table incl. hivly:knowledge:events → hivly:notifier deferred), §13 (config/env split + example), §17 (Deferred: notifier location, Sentry instrumentation)]
...
- [Source: Hivly.config.yml:85-97 — observability + single-tier security block to extend (AC-5)]
...
- **Task 8 (config docs) — real gotcha found and fixed:** `interpolateEnv` substitutes `${VAR}` placeholders across the **entire raw file** before YAML parsing, including inside `#` comments. An initial draft with a commented example like `#   bot_token: "${TELEGRAM_BOT_TOKEN}"` in the shipped `Hivly.config.yml`/`.example` would abort `loadConfig()` on a fresh clone whenever that var is unset — even though the line is a YAML comment and `notifications.enabled` is `false`. Fixed by omitting the `telegram`/`slack` credential blocks entirely while disabled (valid per the `.optional()` schema) and describing the enable-it steps in prose instead of inline `${...}` syntax. Verified by running `loadConfig()` against both the real `Hivly.config.yml` and the tracked `Hivly.config.yml.example` with only the pre-existing required vars set.
- **Also updated `Hivly.config.yml.example`** (the tracked template `git status` doesn't otherwise surface — the real `Hivly.config.yml` is gitignored) with the same 3-tier `rate_limit` + `notifications` block, so a fresh `cp Hivly.config.yml.example Hivly.config.yml` stays valid.
...
- `Hivly.config.yml` (notifications block + 3-tier rate_limit; gitignored, local operator copy)
- `Hivly.config.yml.example` (same reshape, tracked template)
