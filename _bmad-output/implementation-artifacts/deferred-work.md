...
- **Citation/fragment `title: z.string()` accepts `''`** [`packages/shared/src/schemas/citation.ts:16`, `search.ts`/`documents.ts` fragment title]: the strict `link` refine (F1) enforces link quality, but `title` is plain `z.string()` per the ratified D2 convention (same as `channel`/`author`), so an empty title passes the contract, persists, round-trips, and renders as ` — description (link)` in `buildRAGContext` and (future) as an empty 7.5 sources-chip title — undercutting F3's stated "so the chip can show the resource title" intent. Only a *missing* title is rejected, not an empty one. Deferred: consistent with the descriptive-field convention; enrichment bounds title at generation (7.2). Fix, if 7.5 needs the guarantee: `title: z.string().min(1)` on Citation/fragment schemas.
