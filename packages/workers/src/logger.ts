...
// ids instead (see project-context §anti-patterns). AUDIT M2: as a backstop, every
// emitted line is run through the shared `redactSecrets` so a driver error that
// embeds a `user:pass@host` connection URL can't leak it.
...
export function createLogger(level: LogLevel, sink: LogSink = console): Logger {
  ...
  const emit = (msgLevel: LogLevel, message: string, context?: Record<string, unknown>): void => {
    ...
    const prefix = `[workers] ${msgLevel} ${redactSecrets(message)}`;
